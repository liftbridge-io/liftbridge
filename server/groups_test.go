...
// Ensure consumer groups track position in a partition so a consumer can
// resume where it left off.
func TestConsumerGroupDurableConsumer(t *testing.T) {
	defer cleanupStorage(t)
	...
	for i := 0; i < 5; i++ {
		_, err = client.Publish(context.Background(), "foo", []byte(strconv.Itoa(i)))
		require.NoError(t, err)
	}
	...
	// Create consumer.
	cons, err := client.CreateConsumer(group, lift.AutoCheckpoint(0), lift.AutoOffsetEarliest())
	require.NoError(t, err)
	...
	err = cons.Subscribe(context.Background(), []string{"foo"}, func(msg *lift.Message, err error) {
		require.NoError(t, err)
		require.NoError(t, cons.Checkpoint(context.Background()))
		ch <- msg
	})
	require.NoError(t, err)
	...
	// Create consumer that should resume at offset 3.
	cons, err = client.CreateConsumer(group, lift.AutoCheckpoint(0))
	require.NoError(t, err)
	...
	for i := 0; i < 2; i++ {
		select {
		case msg := <-ch:
			require.Equal(t, int64(offset+int64(i)+1), msg.Offset())
		case <-time.After(5 * time.Second):
			t.Fatal("Did not receive expected message")
		}
	}
