...
| encryption.control | | Enable encryption of data stored on server (encryption of data-at-rest). *NOTE: if enabled, an environment variable `LOCAL_MASTER_KEY` must be set to a valid 128 bit or 256 bit AES key.* | bool | false | |
